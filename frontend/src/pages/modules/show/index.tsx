        ...
        <Show aside={<Aside />} title={"Viewing Project"}>
            <SimpleShowLayout>
                <Box display="flex" flexDirection="column" gap="10px">
                    ...
                    <ShowController>
                        {({record}) => (
                            <ReferenceArrayField record={{ id: record.tasks[`key-${record.currentStep}`] }} reference="tasks/assigned" source="id" filter={{ status: "IN_PROGRESS" }} sort={{ field: 'suspense', order: 'ASC' }} >
                                <AssignedTasksField />
                            </ReferenceArrayField>
                        )}
                    </ShowController>
